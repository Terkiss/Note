...
## 추상 메소드(abstract method)
    ...
    선언 되어 있으나 구현되어 있지 않은 메소드 
        abstract 키워드로 선어
        ex 
            public avstract int getValue();
        추상 메소드는 서브 클래스에서 오버라이딩 하여 구현 
    추상 클래스(abstract class)
        추상 메소드를 하나라도 가진 크랠스
        클래스 앞에 반드시 abstract 라고 선언 해야함

    추상 클래스의 인스턴스를 생성 하지 못합니다. 이는 
    오버라이딩 된 메소드를 구현해야 하므로 상속을 통해 구현 화지 않는 이상 인스턴스의 생성은 안됩
    

    abstract 클래스를 상속하는 하위 클래스 

    abstract 클래스의 용도 설계와 구현 분리
    서브클래스마다 목적에 맞게 추상 메소드를 다르게 구현
        다형성 실현
    슈퍼 클래스에서는 개념 정의
        서브 클래스마다 다른 구현이 필요한 메소드는 추상 메소드 로선언
    각서브 클래스에서 구체적인 횅위 를 구현 함 


# 인터페이스의 정의

    인터페이스는 두 결과물의 연결고리가 되는 일조의 약속 역활을 한다.
    인터페이스의 정의로 인해서 홍만군은 홍만군대로
    A사는 a사대로 더이상의 추가 논의 없이 프로젝트를 진행할 수 있엇다.
    인터페이스의 저의 되있기 떄문에 프로젝트를 하나로 묶는 과정동 문제가 되지 않는다.


    메소드 선언부만 존재하는 인터페이스
        인터페이스 
            메소드들의 원형만 나열된 것, 일조의 명세서
            메소드의 원형만 존재하고 몸통은 존재 하지 않음
            메소드들의 몸통은 인터페이스를 구현하는 클래스에서 만드시 구현해야함
            (인터페이스와 클래스 사이의 관계를 구현이라고 함)

            구현 클래스 인터페이스를 구현하는 클래스
            인터페이스는 구현 클래스를 제어하기 위한 목적으로 사용

            인터페이스는 상속 개념이 아님 구현 개념으로 추상 메소드를 구현
            implements
                구현 클래스 와 인터페이스의 관계를 설정하기 위해 사용
            
            
            인터페이스는 상속과 구현이 둘다 가능하다.
            인터페이스는 둘 이상의 인터페이스 구현이 가능하다.
